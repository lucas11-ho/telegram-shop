...
+        <Button
+          variant="secondary"
+          onClick={() => { clearToken(); setMode("login"); }}
+        >
+          Logout
+        </Button>
       </div>
...
+                <div className={ui.muted}>id: {u.id} • telegram: {u.telegram_id || "-"} • admin: {String(u.is_admin)}</div>
               </div>
...
+                <div className={ui.muted}>id: {p.id} • owner: {p.owner_id} • {p.price} {p.currency}</div>
               </div>
...
+                <div className={ui.muted}>user: {o.user_id} • total: {o.total_price} {o.currency} • status: {o.payment_status}</div>
               </div>
               <div className="flex gap-2 flex-wrap justify-end">
-                {["pending","paid","failed","refunded"].map((s) => (
-                  <button key={s} className="rounded border px-2 py-1 hover:bg-gray-50" onClick={()=>setOrderStatus(o.id, s)}>
+                {["pending","paid","failed","refunded"].map((s) => (
+                  <Button key={s} variant="secondary" className="px-3 py-1" onClick={()=>setOrderStatus(o.id, s)}>
                     {s}
-                  </button>
+                  </Button>
                 ))}
